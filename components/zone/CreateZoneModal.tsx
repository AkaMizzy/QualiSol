...
export default function CreateZoneModal({
  ...
  const isDisabled = useMemo(
    () => !title || !token || !projectId || isLimitReached,
    [title, token, projectId, isLimitReached],
  );
  ...
  async function onSubmit() {
    ...
    if (isLimitReached) {
      setError(
        `Vous avez atteint la limite de ${companyInfo?.nbzones || 2} zones. Veuillez mettre à niveau votre plan pour ajouter plus de zones.`,
      );
      return;
    }
    ...
    try {
      ...
      if (e?.message?.includes("limit") || e?.message?.includes("Zone limit")) {
        setError(
          e?.message ||
            `Vous avez atteint la limite de ${companyInfo?.nbzones || 2} zones.`,
        );
      } else {
  ...
  function getMiniMapHtml() {
    ...
    const lat = Number.isFinite(latNum as number)
      ? (latNum as number)
      : 33.5731;
    const lng = Number.isFinite(lngNum as number)
      ? (lngNum as number)
      : -7.5898;
    return `
      <!DOCTYPE html>
  ...
  useEffect(() => {
    const fetchLimitInfo = async () => {
      try {
        ...
        // Filter zones by user's company_id to get accurate count
        // This ensures Super Admin only counts their own company's zones
        const companyZones = zones.filter((z) => z.company_id === company.id);
        setCurrentZonesCount(companyZones.length);

        const limit = company.nbzones || 2;
        setIsLimitReached(companyZones.length >= limit);
      } catch (error) {
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.input,
                        { color: zonetypeId ? "#111827" : "#9ca3af" },
                      ]}
                      numberOfLines={1}
                    >
                      {zonetypeId
                        ? zoneTypes.find(
                            (zt) => String(zt.id) === String(zonetypeId),
                          )?.title || zonetypeId
                        : "Choisir un type (optionnel)"}
                    </Text>
                  </View>
                  <Ionicons
                    name={zoneTypeOpen ? "chevron-up" : "chevron-down"}
                    size={16}
                    color="#9ca3af"
                  />
                </TouchableOpacity>
                  ...
                  >
                    <ScrollView keyboardShouldPersistTaps="handled">
                      {loadingTypes ? (
                        ...
                        <View style={{ padding: 12 }}>
                          <Text style={{ color: "#6b7280" }}>Aucun type</Text>
                        </View>
                      ) : (
                        zoneTypes.map((zt) => (
                          <TouchableOpacity
                            key={zt.id}
                            onPress={() => {
                              setZonetypeId(String(zt.id));
                              setZoneTypeOpen(false);
                            }}
                            style={{
                              paddingHorizontal: 12,
                              paddingVertical: 10,
                              backgroundColor:
                                String(zonetypeId) === String(zt.id)
                                  ? "#f1f5f9"
                                  : "#FFFFFF",
                              borderBottomWidth: 1,
                    ...
                    >
                      {controlId
                        ? companyUsers.find(
                          ...
                          ? `${companyUsers.find((u) => String(u.id) === String(controlId))?.firstname} ${companyUsers.find((u) => String(u.id) === String(controlId))?.lastname || ""}`
                          : controlId
                      ...
                      {isAdmin
                        ...
                        : technicienId
                          ? companyUsers.find(
                            ...
                            ? `${companyUsers.find((u) => String(u.id) === String(technicienId))?.firstname} ${companyUsers.find((u) => String(u.id) === String(technicienId))?.lastname || ""}`
                            : technicienId
