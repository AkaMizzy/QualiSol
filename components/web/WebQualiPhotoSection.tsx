...
interface WebQualiPhotoSectionProps {
  galerieState: ReturnType<
    typeof import("@/hooks/useWebGalerie").useWebGalerie
  >;
}

export default function WebQualiPhotoSection({
  galerieState,
}: WebQualiPhotoSectionProps) {
  const { token } = useAuth();
  const {
    photos,
    loading,
      ...
      <View style={styles.container}>
        ...
        {photos.length === 0 && (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>Aucune qualiphoto disponible</Text>
          </View>
        )}
