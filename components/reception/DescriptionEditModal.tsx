...
export default function DescriptionEditModal({
  ...
  const handleClear = () => {
    Alert.alert("Confirmer", "Voulez-vous vraiment effacer la description ?", [
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await updateGed(token, gedItem.id, {
      ...
      Alert.alert(
        "Erreur",
        error?.message || "Impossible d'enregistrer la description.",
      );
    } finally {
      setIsSaving(false);
    }
